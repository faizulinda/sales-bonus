...
function analyzeSalesData(data, options) {
    ...
    if (!calculateRevenue || !calculateBonus) {
        throw new Error("Не хватает необходимых функций");
    }

    if (typeof calculateRevenue !== "function" || typeof calculateBonus !== "function") {
        throw new Error("Ожидались функции");
    }
    ...
    data.purchase_records.forEach(record => { // Чек 
            ...
            record.items.forEach(item => {
                ...
                const qty = item.quantity; //количество купленного товара
                const unitCost = product.purchase_price;  
                const cost = unitCost * qty; // себестоимость товара
